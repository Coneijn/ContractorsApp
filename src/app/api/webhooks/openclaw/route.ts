...
import { REVERSE_GHL_CONTRACTOR_STAGE_MAP } from '@/lib/ghlMapping'; // Ajusta la ruta a tu carpeta lib si es diferente
import { 
  PrismaClient, 
  TaskStatus, 
  PropertyStatus, 
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if ((target === 'TASK' || target === 'NEW_TASK') && process.env.GHL_API_TOKEN) {
      // Determinamos el estado a buscar en el mapa (las tareas nuevas van a PENDING_ESTIMATE por defecto)
      // Si el newStatus viene vacío pero el target es TASK, asume 'UNASSIGNED' o maneja tu lógica por defecto
      const statusToMap = target === 'NEW_TASK' ? 'PENDING_ESTIMATE' : (newStatus || 'UNASSIGNED');
      // Buscamos el ID del stage en GHL usando nuestro mapa inverso
